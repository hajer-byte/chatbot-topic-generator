...
import useUser from 'plugins/account/hooks/user';
import { GenerateIdeasContext } from './context/GenerateIdeasContext';
import SimpleBar from 'simplebar-react';

const GenerateIdeaFullView = () => {
  ...
  return (
    <SimpleBar style={{ height: '850px' }} className="page-title-box">
      <h1 className="ms-3 page-title d-inline-block">{translations['menu.topicFinder']}</h1>
      <Card>
        <GenerateIdeaHeader ideaMessagesIndex={ideaMessagesIndex} handleResetChat={handleResetChat} />
        ...
        >
          <GenerateIdeaBody handleIdeaMessages={handleIdeaMessages} handleIdeaMessagesIndex={handleIdeaMessagesIndex} />
        </GenerateIdeasContext.Provider>
      </Card>
    </SimpleBar>
  );
...
export default GenerateIdeaFullView;
